...
import type { CreatePornstarData, Country } from '@/types/pornstar';
import ProtectedRoute from '@/components/ProtectedRoute';
import CountrySelect from '@/components/CountrySelect';

export default function CreatePornstarPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await pornstarService.create(submitData);
      router.push('/pornstars');
    } catch (err: any) {
      setError(err.response?.data?.message || 'Failed to create pornstar');
      setSubmitting(false);
    }
  };
          ...
          <div className="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-800">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                >
                  {submitting ? 'Creating...' : 'Create Pornstar'}
                </button>
              </div>
            </form>
          </div>
